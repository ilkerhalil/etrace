...
namespace etrace
{
    ...
    internal class Options
    {
        [Option("raw", Required = false, MutuallyExclusiveSet = "filter",
            HelpText =
                "Regular expression to match against the entire event description. This is not very efficient; prefer using --where if possible."
        )]
        public string RawFilter { get; set; } = "";

        [OptionList("where", Required = false, Separator = ',', MutuallyExclusiveSet = "filter",
            HelpText =
                "Filter payload fields with a regular expression. For example: ImageFileName=notepad,ParentID=4840"
        )]
        ...
        [Option("list", Required = false,
            HelpText =
                "List keywords and/or providers. Options include: CLR, Kernel, Registered, Published, or a comma-separated combination thereof."
        )]
        ...
        [OptionList("field", Required = false, Separator = ',',
            HelpText =
                "Display only these payload fields (if they exist). The special fields Event, PID, TID, Time can be specified for all events. An optional width specifier can be provided in square brackets. For example: PID,TID,ProcessName[16],Receiver[30],Time"
        )]
        ...
        public long ParsedClrKeywords { get; private set; }

        public KernelTraceEventParser.Keywords ParsedKernelKeywords { get; private set; } =
        ...
        public void PostParse()
        {
            ...
            foreach (var keyword in ClrKeywords)
                ParsedClrKeywords |=
                    (long) (ClrTraceEventParser.Keywords) Enum.Parse(typeof(ClrTraceEventParser.Keywords), keyword);
            foreach (var keyword in KernelKeywords)
                ParsedKernelKeywords |=
                    (KernelTraceEventParser.Keywords) Enum.Parse(typeof(KernelTraceEventParser.Keywords), keyword);
        }
        ...
        public class Filter
        {
                ...
                if (string.Equals(Key, key, StringComparison.OrdinalIgnoreCase))
                    switch (OperatorChar)
                    {
                        ...
                        case DOUBLE_EQUAL_SIGN:
                            result = string.Equals(RawValue, value, StringComparison.OrdinalIgnoreCase);
                            break;
                        default:
                            break;
                    }
            ...
            private bool IsPayloadMatch(TraceEvent e)
            {
                ...
                var payloadValue = e.PayloadByName(Key);
        ...
        private Filter ParseFilter(string filter)
        {
            ...
            if (filter.Contains(MultipleFilter.MULTIPLE_FILTER_SIGN))
            {
                ...
                var multiFilter = new MultipleFilter();
        ...
        private string[] SplitMultipleFilter(string filter, string splitby)
        {
            ...
            var result = filter.Split(new[] {splitby}, StringSplitOptions.RemoveEmptyEntries);
